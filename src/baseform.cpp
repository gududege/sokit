...
#define PROP_TIMER "timer"
#define PROP_BTN "button"
#define SIG_BTN "released"
#define SET_MAX_PERIOD 999999
...
void BaseForm::bindTimer(QLineEdit* e, QCheckBox* c, QTimer* t, QToolButton* s)
{
    e->setValidator(new QIntValidator(1, SET_MAX_PERIOD, this));


    c->setProperty(PROP_EDIT, qVariantFromValue((void*)e));
    c->setProperty(PROP_TIMER, qVariantFromValue((void*)t));
    connect(c, SIGNAL(stateChanged(int)), this, SLOT(toggleTimer(int)));

    t->setProperty(PROP_BTN, qVariantFromValue((void*)s));
    connect(t, SIGNAL(timeout()), this, SLOT(send_repeat()));

}

void BaseForm::toggleTimer(int state)
{
    QLineEdit* e = (QLineEdit*)sender()->property(PROP_EDIT).value<void*>();
    QTimer* t = (QTimer*)sender()->property(PROP_TIMER).value<void*>();

    if (state == Qt::Checked) {
        QString input = e->text();        
        bool ok;
        int interval = input.toInt(&ok);
        if (ok) {
            e->setEnabled(false);
            t->start(interval);
        } else {
...
void BaseForm::send_repeat()
{
    QToolButton* s = (QToolButton*)sender()->property(PROP_BTN).value<void*>();
    QMetaObject::invokeMethod(s, SIG_BTN, Qt::QueuedConnection);
}
